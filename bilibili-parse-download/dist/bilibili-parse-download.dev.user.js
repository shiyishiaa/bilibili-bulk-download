...
function getSignData(param, sec) {
  return common_objectSpread(common_objectSpread({}, param), {}, {
    sign: md5("".concat(Object.entries(param).filter(function (_ref2) {
      var _ref3 = common_slicedToArray(_ref2, 1),
        k = _ref3[0];
      return k !== 'sign';
    }).map(function (e) {
...
function download_all() {
  ...
  MessageBox.confirm(msg, function () {
    ...
    if (dl_subtitle) {
      ...
      if (video_tasks.length === 1) {
        ...
        download_subtitle_vtt_zip([].concat(video_tasks), new JSZip());
      }
    }
    if (dl_danmaku) {
      // 3.下载弹幕
      if (video_tasks.length === 1) {
        ...
        download_danmaku_ass_zip([].concat(video_tasks), new JSZip());
      }
    }
  });
  ...
  if (config_config.video_list_auto_scroll_load !== '0') {
    ...
    timer2 = setInterval(function () {
      ...
      if (h > old_h) {
        !change_count && MessageBox.confirm('正在加载视频列表，请稍后...', null, function () {
          clearInterval(timer1);
...
function download_api(url, filename) {
  var api = config_config.download_api;
  var api_sec = config_config.download_api_secret;
  if (!api) {
    message_Message.warning('请配置下载接口');
    return;
  }
  var data = {
    filename: filename,
...
var config_functions = {
  ...
  ariang_sync_config: function ariang_sync_config() {
    ...
    var ariang_window = window.bp_aria2_window;
    if (ariang_window && !ariang_window.closed) {
      ariang_window.location.href = config_config.ariang_host + Download.get_ariang_set_hash(rpc);
      message_Message.info('已同步配置');
      return;
    }
